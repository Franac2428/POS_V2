  ...
  image
    ...
    const handleChangeQuantity = (e, id, name) => {
        ...
        if (name === "cantidad") {
          ...
          } else if (newQuantity > cantProducto) {
            toast.error("No puede ordenar una cantidad mayor a la disponible");
            newQuantity = cantProducto; // Limitar al máximo permitido
          }
      
          // Actualizamos el valor siempre que no sea eliminado
          e.target.value = newQuantity; // Sincroniza el valor en el campo de entrada
          onChange({ target: { value: newQuantity } }, id, name);
        }
      };
    ...
    <div className="grid grid-cols-3 items-center bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
      ...
      <div className="flex flex-col items-start justify-between p-2 space-y-2">
        ...
        <div className="flex items-center space-x-2">
          <button
            type="button"
            onClick={() => handleChangeQuantity({ target: { value: quantity } }, id, 'cantidad')}
      ...
      <div className="flex flex-col items-end justify-between p-2 space-y-2">
        <div className="flex items-center space-x-1">
          <span className="text-gray-00 dark:text-gray-400 text-xs font-medium">₡</span>
          <span className="text-gray-900 text-xs rounded-md  font-medium block w-16 p-1">{price}</span>
        </div>
        <HtmlTableButton
